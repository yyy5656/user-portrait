...
import ImageRegister from "../img/register.jpg";
...
function Login() {
  ...
  const [imgSrc, setImgSrc] = useState(ImageLogin); // 图片地址
  ...
  const register = (values) => {
    ...
    const promise = new Promise((resolve, reject) => {
      // 校验用户名是否相同
      api.checkUsername({ username: values.register_username }).then((res) => {
        console.log(res);
